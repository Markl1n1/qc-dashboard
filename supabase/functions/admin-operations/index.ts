...
async function verifyAdminRole(supabase: any, userId: string): Promise<{ isAdmin: boolean; error: Response | null }> {
  ...
  if (profile.role !== 'admin') {
    console.error('Security Event: Non-admin user attempted admin operation:', userId);
    return {
      isAdmin: false,
      error: new Response(
...
serve(async (req) => {
  ...
  try {
    ...
    // Verify admin role for all admin operations
    const { isAdmin, error: roleError } = await verifyAdminRole(supabase, user.id);
    if (roleError) {
      return roleError;
    }

    console.log('Security Event: Admin operation authorized for user:', user.id);
