...
const Upload: React.FC<UploadProps> = () => {
  ...
  useEffect(() => {
    if (audioFiles.length === 0) { setSignalMetrics(null); return; }
    const runAnalysis = async () => {
      setIsAnalyzingAudio(true);
      try {
        const metrics = await analyzeAudioSignal(audioFiles[0]);
        setSignalMetrics(metrics);
      } catch (e) { console.warn('Audio analysis failed:', e); setSignalMetrics(null); }
  ...
  const onDrop = useCallback((acceptedFiles: File[]) => {
    ...
    if (oversizedFiles.length > 0) {
      ...
      if (validFiles.length > 0) setAudioFiles(prev => [...prev, ...validFiles]);
      return;
    }
  ...
  const handleFileReorder = useCallback((reorderedFiles: File[]) => { setAudioFiles(reorderedFiles); }, []);
  const handleFileRemove = useCallback((index: number) => { setAudioFiles(prev => prev.filter((_, i) => i !== index)); }, []);

  const handleTranscribe = async () => {
    if (audioFiles.length === 0) { toast.error(t('upload.uploadAtLeastOne')); return; }
    if (!agentName.trim()) { toast.error(t('upload.selectAgent')); return; }
    if (!user) { toast.error(t('upload.mustBeLoggedIn')); return; }

    deepgramService.setNoiseReduction(noiseReduction);
    let dialogId: string | null = null;
    
    try {
      const originalFileName = audioFiles.length === 1 ? audioFiles[0].name : `merged_${audioFiles.length}_files.mp3`;
      const newFileName = generateFileName(agentName.trim(), originalFileName);

      dialogId = await addDialog({
        fileName: newFileName, status: 'processing', assignedAgent: agentName.trim(),
        assignedSupervisor: user.email || 'Unknown', uploadDate: new Date().toISOString(),
        tokenEstimation: { audioLengthMinutes: 0, estimatedCost: 0 }, isSegmented: false, currentLanguage: 'original'
      });

      if (signalMetrics && dialogId) {
        try {
          const { supabase } = await import('../integrations/supabase/client');
          await (supabase as any).from('dialogs').update({ audio_quality_metrics: signalMetrics }).eq('id', dialogId);
        } catch (e) { console.warn('Failed to save audio quality metrics:', e); }
      }

      let fileToTranscribe: File;
      if (audioFiles.length > 1) {
        setIsMerging(true); setMergingProgress(null);
        serverAudioMergingService.setProgressCallback((progress) => { setMergingProgress(progress); });
      ...
      const deepgramOptions: DeepgramOptions = {
        model: 'nova-2-general', language: selectedLanguage, speaker_labels: true,
        smart_formatting: true, profanity_filter: false, punctuation: true
      };
      ...
      await updateDialog(dialogId, { status: 'completed', audioLengthMinutes: audioDurationMinutes });
      toast.success(t('upload.transcriptionComplete'));
      navigate(`/dialog/${dialogId}`);
    } catch (err: any) {
      console.error('Transcription failed', err);
      toast.error(`${t('upload.transcriptionFailed')}: ${err.message}`);
      if (dialogId) await updateDialog(dialogId, { status: 'failed', error: err.message });
    } finally { setIsMerging(false); }
  };

  const handleCreateNewAgent = () => { navigate('/agents'); };
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2"><User className="h-5 w-5" />{t('upload.agentInfo')}</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="space-y-4">
            <AgentSelector value={agentName} onChange={setAgentName} onCreateNew={handleCreateNewAgent} />
            <div>
              <Label>{t('upload.supervisor')}</Label>
              <Input value={user?.email || t('upload.notLoggedIn')} disabled className="max-w-md bg-muted" />
            </div>
          </div>
        </CardContent>
      </Card>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2"><Volume2 className="h-5 w-5" />{t('upload.noiseReduction')}</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="flex items-center justify-between">
              <div>
                <Label htmlFor="noise-reduction" className="font-medium">{t('upload.rnnoiseDenoising')}</Label>
                <p className="text-sm text-muted-foreground mt-1">{t('upload.noiseReductionDesc')}</p>
              </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2"><Mic className="h-5 w-5" />{t('upload.uploadAudio')}</CardTitle>
        </CardHeader>
        <CardContent>
          <div {...getRootProps()} className="flex flex-col items-center justify-center w-full h-48 border-2 border-dashed rounded-md cursor-pointer bg-muted hover:bg-accent">
            ...
            {isDragActive ? (
              ...
              <div className="text-center">
                ...
                <p className="text-xs text-muted-foreground mt-1">{t('upload.multiMerge')}</p>
                <p className="text-xs text-muted-foreground mt-1">{t('upload.maxFileSize')}</p>
              </div>
            )}
          </div>
          <DraggableFileList files={audioFiles} onReorder={handleFileReorder} onRemove={handleFileRemove} />
          {(isAnalyzingAudio || signalMetrics) && audioFiles.length > 0 && (
            <div className="mt-4"><AudioQualityIndicator signalMetrics={signalMetrics} isAnalyzing={isAnalyzingAudio} /></div>
          )}
          {audioFiles.length > 1 && (
            <div className="mt-4 p-3 bg-accent border border-border rounded-lg">
              <div className="flex items-start gap-2">
                ...
                <div className="text-sm">
                  <p className="font-medium text-foreground">{t('upload.serverMerge')}</p>
                  <p className="text-muted-foreground">{t('upload.serverMergeDesc')}</p>
                </div>
              </div>
            </div>
          )}
      ...
      <div className="flex justify-center">
        <Button onClick={handleTranscribe} disabled={!canTranscribe} size="lg">
          {isMerging ? t('upload.mergingFiles') : isDeepgramLoading ? t('upload.transcribing') : t('upload.startTranscription')}
        </Button>
      </div>

      {isMerging && mergingProgress && (
        <Card><CardHeader><CardTitle>{t('upload.mergingAudio')}</CardTitle></CardHeader>
          <CardContent><p>{mergingProgress.message}</p><Progress value={mergingProgress.progress} /></CardContent></Card>
      )}

      {isDeepgramLoading && audioFiles.length > 1 && (
        <MultiFileTranscriptionProgress files={audioFiles} currentFileIndex={0} currentFileProgress={deepgramProgress?.progress || 0}
          overallProgress={deepgramProgress?.progress || 0} stage={deepgramProgress?.stage || 'processing'} message={deepgramProgress?.message || 'Processing...'} />
      )}

      {isDeepgramLoading && audioFiles.length === 1 && deepgramProgress && (
        <Card><CardHeader><CardTitle>{t('upload.transcriptionProgress')}</CardTitle></CardHeader>
          <CardContent><p>{deepgramProgress.message}</p><Progress value={deepgramProgress.progress} /></CardContent></Card>
      )}

      {deepgramError && (
        <Card><CardHeader><CardTitle>{t('upload.transcriptionError')}</CardTitle></CardHeader>
          <CardContent><p className="text-destructive">{deepgramError}</p></CardContent></Card>
      )}
    </div>
  );
};
